...
export default function Navbar() {
  ...
  const links = user?.role === "admin"
    ? [{ to: "/admin", label: "Dashboard" }, { to: "/dashboard", label: "All Complaints" }]
          ...
          <>
            ...
            {links.map(({ to, label }) => (
              <Link key={to} to={to} style={{
                ...
                background: location.pathname === to ? (user?.role === "admin" ? "rgba(245,158,11,0.1)" : "rgba(56,189,248,0.1)") : "transparent",
                border: location.pathname === to ? `1px solid ${user?.role === "admin" ? "rgba(245,158,11,0.2)" : "rgba(56,189,248,0.2)"}` : "1px solid transparent",
                transition: "all 0.2s",
              }}>{label}</Link>
            ))}
