      ...
      <div style={{ background: "#1e293b", borderRadius: "16px", border: "1px solid rgba(239,68,68,0.2)", padding: "28px", width: "100%", maxWidth: "400px", fontFamily: font }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <input
            ...
            placeholder="e.g. Spam complaints, Inappropriate behaviour"
            value={reason}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "#1e293b", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)", padding: "28px", width: "100%", maxWidth: "440px", fontFamily: font }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "20px" }}>
          ...
          <div>
            <div style={{ fontSize: "17px", fontWeight: "700", color: "#f1f5f9", letterSpacing: "-0.3px" }}>{student.name}</div>
            <div style={{ fontSize: "12px", color: "#64748b", marginTop: "2px" }}>{student.email}</div>
            <div style={{ display: "flex", gap: "6px", marginTop: "6px", flexWrap: "wrap" }}>
              {student.isBanned && (
                <span style={{ fontSize: "10px", color: "#f87171", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", padding: "2px 8px", borderRadius: "10px", fontWeight: "600" }}>🚫 BANNED</span>
              )}
              {student.isVerified
        ...
        <div style={{ background: "#0f172a", borderRadius: "10px", padding: "14px", marginBottom: "16px" }}>
          {[
            ...
            student.isBanned && { label: "Ban Reason", val: student.banReason || "No reason given" },
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          {student.isBanned ? (
            <button onClick={() => { onUnban(student._id); onClose(); }}
              style={{ flex: 1, padding: "11px", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: "9px", color: "#10b981", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: font }}>
              ✓ Unban Student
            </button>
          ) : (
            <button onClick={() => { onClose(); onBan(student); }}
              style={{ flex: 1, padding: "11px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "9px", color: "#f87171", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: font }}>
              🚫 Ban Student
            </button>
          )}
          <button onClick={onClose}
            style={{ flex: 1, padding: "11px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "9px", color: "#64748b", fontSize: "13px", cursor: "pointer", fontFamily: font }}>
            Close
          </button>
        </div>
      </div>
...
export default function StudentsPage() {
  ...
  const tabs = [
    ...
    { key: "Active", label: `Active (${verified - banned > 0 ? verified - banned : 0})` },
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "10px", marginBottom: "24px" }}>
          {[
            ...
            { label: "Active",     val: verified - banned < 0 ? 0 : verified - banned, color: "#10b981" },
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {filtered.map(s => (
              <div key={s._id} style={{ background: "#1e293b", borderRadius: "12px", border: `1px solid ${s.isBanned ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.06)"}`, padding: "14px 16px", display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap", transition: "border 0.2s" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "3px" }}>
                    ...
                    {s.isBanned && (
                      <span style={{ fontSize: "10px", color: "#f87171", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", padding: "1px 7px", borderRadius: "10px", fontWeight: "600" }}>BANNED</span>
                    )}
                    {!s.isVerified && (
                      <span style={{ fontSize: "10px", color: "#f59e0b", background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", padding: "1px 7px", borderRadius: "10px", fontWeight: "600" }}>UNVERIFIED</span>
                    )}
                    {s.isVerified && !s.isBanned && (
                      <span style={{ fontSize: "10px", color: "#10b981", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.2)", padding: "1px 7px", borderRadius: "10px", fontWeight: "600" }}>ACTIVE</span>
                    )}
                  </div>
